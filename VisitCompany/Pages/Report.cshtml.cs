...
namespace VisitCompany.Pages
{
    public class ReportModel : PageModel
    {
        ...
        public int StatusAssignedCountForweek { get; set; }
        public int StatusAssignedCountForMonth { get; set; }
        ...
        public bool SearchAccounts { get; set; }
        ...
        public List<(string PropertyName, long TotalScore, int WeakCompanyCount)> TopScores { get; set; } = new List<(string, long, int)>();
        ...
        public void OnGet()
        {
            ...
            var checklistData = _checklistApplication.Serach(
            ...
            var companyData = _company.Serach(
    ...
    .ToList() ?? new List<MergedData>(); // مقداردهی پیش‌فرض
            ...
            StatusAssignedCountForweek = Companies.FirstOrDefault()?.StatusAssignedForWeekCount ?? 0;  // برای نمایش تعداد ارجاع های داده شده در یک هفته اخیر و پایینی برای ماه اخیر
            StatusAssignedCountForMonth = Companies.FirstOrDefault()?.StatusAssignedForMonthCount ?? 0;
            ...
            // نمره زیر 50 میشه امتیاز کم این میاد توی نمره های شرکت ها اونایی که میانگینشون کمترین هست رو پیدا میکنه حالا میاد میکه در بین چند تا شرکت این ضعف بوده  نمره زیر 50 هم میشه ضعف 
            //عدد زیر 50 رو ضعف میگیره و دربین شرکت های ضغیف اون تعداد نمره که زیر 50 هست رو میشماره و نشون میده
            var scores = _generalChecklistApplication.GetMostVulnerableProperties();
            TopScores = scores ?? new List<(string, long, int)>();
        ...
        [HttpPost]
        public IActionResult OnPost(string keyword, bool searchCompanies, bool searchChecklists, bool searchLicences, bool searchPersons, bool searchAccounts)

        {
            ...
            TempData["SearchAccounts"] = searchAccounts;
    ...
    .ToList() ?? new List<MergedData>(); // مقداردهی پیش‌فرض
            ...
            if (!string.IsNullOrWhiteSpace(Keyword))
            {
                ...
                if (searchAccounts)
                {
                    AccountsSearch = _accountApplication.SearchTotal(new AccountSearchModel { Fullname = Keyword, UserName = Keyword, Mobile = Keyword }, currentUserRole == Convert.ToInt64(RolesConst.State) ? currentUserProvinceId : (long?)null);
                }
            }
